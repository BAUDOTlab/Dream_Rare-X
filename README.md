...
Furthermore, we leverage the genetic information from Orphanet, specifically the mutated genes associated with diseases, to establish connections between *Orphanet diseases* within the layer. Two *Orphanet diseases* are linked with an edge weighted `1` if they share at least one mutated gene as documented in the Orphanet database. This strategic approach facilitates MultiXrank's navigation between *Orphanet diseases*, guided by the assumption that diseases sharing the same causative gene may possess common (and potentially unreported) phenotypes.
