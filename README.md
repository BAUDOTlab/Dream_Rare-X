...
Rare diseases often pose significant diagnostic challenges due to their low prevalence and the diversity of their associated symptoms. Gaining a more comprehensive perspective on the symptoms and phenotypes associated with rare diseases is crucial, as it could facilitate the early detection and more efficient management of these uncommon conditions.
...
The first layer, hereafter referred to as the *Rare-X layer*, contains information extracted from the *Rare-X* dataset of symptoms associated with a cohort of rare disease patients. This layer contains three node types:
...
The second layer, hereafter referred to as the *Orphanet layer*, contains information extracted from the Orphanet and HPO databases. It contains two types of nodes:
...
After building the multilayer network, we can run MultiXrank, taking the *Rare-X disease* nodes iteratively as seeds, as illustrated by the blue node in the above figure. Starting from a *Rare-X disease* node, the random walker can navigate within and across the two layers and score each node according to its relevance with respect to the seed node.

By comparing top scored *Symptom* nodes from the Rare-X layer with the top scored *Phenotype* nodes from the Orphanet layer we expect to reveal novel symptoms associated with rare diseases that are not reported in Orphanet.
...
MultiXrank returns one file for each layer of the multilayer network. Those files contain the scores associated with each node composing the layer considered. To enhance result readability, we extract the nodes with the highest scores from each layer and consolidate them in a single file.

The code used to parse MultiXrank results is provided in the `step5_Parse_MultiXrank_Results.ipynb` jupyter-notebook.

*Note:*  The raw MultiXrank output files are stored in the folder `results_MultiXrank/output/`.
Processed output files are stored in the folder `results_MultiXrank/results/`.
